...
namespace gr
{
  namespace lora_sdr
  {
    ...
    public:
      preamble_sync_impl(uint32_t center_freq, uint32_t bandwidth, uint8_t sf, bool impl_head, std::vector<uint16_t> sync_word, uint8_t os_factor, uint16_t preamb_len);
      ~preamble_sync_impl();
