...
namespace gr {
    namespace lora_sdr {
        ...
        fft_demod::sptr
        fft_demod::make(uint8_t sf, bool impl_head, bool soft_decoding, bool max_log_approx) {
            return gnuradio::get_initial_sptr(new fft_demod_impl(sf, impl_head, soft_decoding, max_log_approx));
        }

        /*
         * The private constructor
         */
        fft_demod_impl::fft_demod_impl(uint8_t sf, bool impl_head, bool soft_decoding, bool max_log_approx)
            : gr::block("fft_demod",
                        gr::io_signature::make(1, 1, (1u << sf) * sizeof(gr_complex)),
                        gr::io_signature::make(0, 1, soft_decoding ? sf * sizeof(LLR) : sizeof(uint32_t))),
                        m_sf(sf), m_soft_decoding(soft_decoding), max_log_approx(max_log_approx), 
                        m_new_frame(true) {
        ...
        void fft_demod_impl::new_frame_handler(int cfo_int) {
            ...
            m_new_frame = true; // Notify for a new frame (SNR computation)
        };
        ...
        int32_t fft_demod_impl::get_symbol_val(const gr_complex *samples) {
            ...
            // Return argmax
            int idx = std::max_element(m_fft_mag_sq, m_fft_mag_sq + m_samples_per_symbol) - m_fft_mag_sq;
        ...
        int fft_demod_impl::general_work(int noutput_items,
            ...
            if (output.size() == block_size || LLRs_block.size() == block_size) {
                if (m_soft_decoding) {
                    ...
                    memcpy(out1, output.data(), block_size * sizeof(uint32_t));
                    output.clear();
                }
