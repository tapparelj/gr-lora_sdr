...
#include "frame_detector_impl_old.h"
#include "helpers.h"
...
int frame_detector_impl::general_work(int noutput_items,
  ...
  default: {
    ...
    int length = m_samples_per_symbol;
    float rat = ninput_items[0] / m_samples_per_symbol;
    int sym = std::floor(rat);
    std::vector<gr_complex> temp;
    temp.resize(m_samples_per_symbol);
    for (int j = 0; j < sym; j++) {

      for (int k = 0; k < m_samples_per_symbol; k++) {
        temp[k] = in[k + m_samples_per_symbol * j];
      }
      //      float power_temp;
      //      power_temp = calc_power(&temp[0], m_samples_per_symbol);
      // check with the current input if we are still in a LoRa frame
      bool in_frame = check_in_frame(&temp[0], length);
    ...
    // return the number of items produced by the system to the scheduler
    return end_vec;
  }
