  ...
  namespace lora_sdr {
    ...
    frame_sync_impl::frame_sync_impl(float samp_rate, uint32_t bandwidth, uint8_t sf, bool impl_head, std::vector<uint16_t> sync_word)
      ...
      gr::io_signature::make(0, 1, (1u << sf)*sizeof(gr_complex)))
      {
        ...
        n_up                = 8;
        n_up_trig           = n_up-3;
        preamb_up_vals.resize(n_up_trig,0);
        
        up_symb_to_use      = n_up_trig-1;
    ...
    void frame_sync_impl::frame_info_handler(pmt::pmt_t frame_info){
        ...
        m_ldro = pmt::to_bool(pmt::dict_ref(frame_info,pmt::string_to_symbol("ldro"),err));
        ...
        if(m_invalid_header){
            ...
            add_item_tag(0, nitems_written(0) ,pmt::string_to_symbol("frame_info"),frame_info);
                // std::cout<<"add payload tag at "<< nitems_written(0)<<"\n";
                       ...
                       gr_vector_void_star &output_items)
    {
        ...
        switch (m_state) {
          case DETECT: {
            ...
            //   } 
              if(std::abs(bin_idx_new-bin_idx)%((1u<<m_sf)-1)<=1 && bin_idx_new!=-1){//look for consecutive reference upchirps(with a margin of ±1)
          ...
          case SYNC:{
              if(!cfo_sto_est){
                  ...
                  float lambda_bernier = estimate_CFO_frac_Bernier(&preamble_raw[m_number_of_bins-k_hat]); //TODO choose one cfo est
                  estimate_STO();
              ...
              switch (symbol_cnt) {
                  ...
                  case NET_ID1:{
                    ...
                    case QUARTER_DOWN:{
                        ...
                        #endif
                        print("CFO = "<<lambda_cfo+CFOint<< " CFOBernier = "<<lambda_bernier);
        ...
        // std::cout<<"ninput_items "<<ninput_items[0]<<", items_to_consume "<<items_to_consume<<", noutput_items "<<noutput_items<<", items_to_output "<<items_to_output<<std::endl;
