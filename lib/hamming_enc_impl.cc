...
namespace gr
{
  namespace lora_sdr
  {
    ...
    hamming_enc::sptr
    hamming_enc::make(uint8_t cr, uint8_t sf, bool legacy_sf56 = false)
    {
      return gnuradio::get_initial_sptr(new hamming_enc_impl(cr, sf, legacy_sf56));
    }

    /*
     * The private constructor
     */
    hamming_enc_impl::hamming_enc_impl(uint8_t cr, uint8_t sf, bool legacy_sf56)
                           ...
                           gr_vector_void_star &output_items)
    {
      ...
      for (int i = 0; i < nitems_to_process; i++)
      {
        ...
        uint8_t cr_app;
        if(m_legacy_sf56){
          cr_app = (m_cnt < m_sf-2) ? 4 : m_cr;
        }
        else{
          cr_app = (m_cnt < m_sf - (m_sf < 7?0:2)) ? 4 : m_cr; //for sf <7  we don't use the ldro
        }
