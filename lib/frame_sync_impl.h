...
namespace gr
{
  namespace lora_sdr
  {
    ...
    private:
      ...
      std::vector<gr_complex> received_preamble; ///< new synchronization vector, contains all preamble (upchirps, net ids and downchirps)
      ...
      float m_sfo_hat;                             ///< estimated sampling frequency offset
      ...
      std::ofstream debug_file2;
      std::ofstream debug_file3;
      std::ofstream debug_file4;
      bool first = true;
#endif
       ...
       *          first symbol since it might be incomplete)
       */
      float estimate_CFO_frac_Xhonneux(gr_complex *samples, int symb_to_use);
      /**
